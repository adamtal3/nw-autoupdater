const { join } = require("path"),
    fs = require("fs-extra"), { launch, copy } = require("../utils"), { swapFactory, IS_OSX } = require("../env");
...
async function restartToSwap() {
    ...
    if (IS_OSX) {
        await launch("open", [app, "--args", ...args], updateDir, logPath);
    } else {
...
async function restart() {
    ...
    if (IS_OSX) {
        await launch("open", [app, "--args"], execDir, logPath);
    } else {
