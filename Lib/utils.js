...
async function launch(runnerPath, argv, cwd, logPath) {
    return new Promise((resolve, reject) => {
        ...
        if (IS_OSX) {
            exec(`${runnerPath} ${argv.join(' ')}`, (error, stdout, stderr) => {
                if (error) {
                    fs.writeSync(log, ["ERROR:", error, "\r\n"].join(" "), "utf-8");
                    reject(log);
                }

                if (stdout) {
                    fs.writeSync(log, `${stdout}`, "utf-8");
                }

                if (stderr) {
                    fs.writeSync(log, `ERROR: ${stderr}`, "utf-8");
                }
            });
        } else {
            const child = spawn(runnerPath, argv, {
                timeout: 4000,
                detached: true,
                cwd
            });
